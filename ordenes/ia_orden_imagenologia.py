# ordenes/ia_orden_imagenologia.py
# Orden de Imagenología para MÓDULO IA — timbre rotado 20°
...
logger = logging.getLogger("ia_orden_imagenologia")
...
def _estilos():
    ...
    return titulo, subtitulo, campo, examen_label, examen_val, nota_s
...
# ============================================================
def generar_orden_imagenologia_ia(datos: dict, resolver_derivacion=None) -> bytes:
    """
    datos: {
        nombre, edad, rut, dolor, lado,
        examen: str,   ← string final normalizado por el router (IA)
        nota?:  str,   ← nota construida en el router
        geo?:   dict,  ← para resolver derivación
    }
    resolver_derivacion: callable opcional — función del módulo resolver
    ...
    # — Nota desde derivación —
    nota_texto = ""
    if resolver_derivacion and callable(resolver_derivacion):
        try:
            deriv = resolver_derivacion({**datos, "examen": examen, "dolor": dolor}, geo) or {}
            nota_raw = deriv.get("nota") or ""
            if isinstance(nota_raw, str) and nota_raw.strip():
                nota_texto = f"Nota:\n\n{nota_raw.strip()}"
        except Exception as e:
            logger.warning("resolver_derivacion falló: %s", e)
    elif datos.get("nota"):
        nota_texto = str(datos["nota"]).strip()

    titulo_s, subtitulo_s, campo_s, examen_label_s, examen_val_s, nota_s = _estilos()
    ...
    elementos.append(Paragraph("Orden Médica de Imagenología (IA)", subtitulo_s))
    ...
    # — Nota —
    if nota_texto:
        elementos.append(Paragraph(nota_texto.replace("\n", "<br/>"), nota_s))
        elementos.append(Spacer(1, 6 * mm))
